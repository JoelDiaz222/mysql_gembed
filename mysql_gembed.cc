...
static bool embed_text_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
    if (args->arg_count != 3) {
        snprintf(message, MYSQL_ERRMSG_SIZE,
                "EMBED_TEXT requires 3 arguments: backend, model, text");
        return true;
    }
...
static char *embed_text(UDF_INIT *initid, UDF_ARGS *args,
                        char * /*result*/, unsigned long *length,
                        unsigned char *is_null, unsigned char *error) {
    const char *backend = args->args[0];
    const char *model = args->args[1];
    const char *text = args->args[2];

    if (!backend || !model || !text) {
        *is_null = 1;
        return nullptr;
    }

    int backend_id = validate_backend(backend);
    if (backend_id < 0) {
        *error = 1;
        log_message(ERROR_LEVEL, "Invalid embedding backend");
        return nullptr;
    }

    int model_id = validate_model(backend_id, model, INPUT_TYPE_TEXT);
    if (model_id < 0) {
        *error = 1;
    ...
    int err = generate_embeddings(backend_id, model_id, &input_data, &batch);
...
/* UDF: EMBED_TEXTS(backend, model, JSON_ARRAY(texts)) -> JSON_ARRAY(vectors) */
static bool embed_texts_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
    if (args->arg_count != 3) {
        snprintf(message, MYSQL_ERRMSG_SIZE,
                "EMBED_TEXTS requires 3 arguments: backend, model, texts_json");
        return true;
    }
...
static char *embed_texts(UDF_INIT *initid, UDF_ARGS *args,
    ...
    if (backend_id < 0) {
        *error = 1;
        log_message(ERROR_LEVEL, "Invalid embedding backend in batch");
        return nullptr;
    }
